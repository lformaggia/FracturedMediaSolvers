...
public:
    ...
    */
    MatrixHandler(const Rigid_Mesh & pmesh, const BoundaryConditions & bc,
		const UInt mdim, const UInt brow, const UInt bcol, const UInt tdim):
        M_mesh(p_mesh), M_bc(bc), Mdim(mdim), Bdim(brow,bcol), Tdim(tdim), 
        M(new SpMat(mdim,mdim)), B(new SpMat(brow,bcol)), T(new SpMat(tdim,tdim),
        M_b(new Vector(Vector::Constant(mdim + brow, 0.))){}
    ...
    void showMe() const
        {
			std::cout<<std::endl;
			std::cout<<"The system dimension is : "<<Mdim+Bcol<<std::endl<<std::endl;
		};
