...
bool
FVCode3D::dumpSaddlePointMatrix (const SaddlePointMat& M)
{
  bool status{true};
...
bool
FVCode3D::dumpApproximateSchurMatrix (const SaddlePointMat& SP, bool lumping)
{
  ...
  if (lumping)
     ...
     for (int i=0; i< MRed.rows();++i)trip.emplace_back(i,i,MRed.diagonal()[i]);
