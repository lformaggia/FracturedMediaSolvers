...
void MassMatrix::assemble()
{
	...
	for (auto cell_it : this->M_mesh.getCellsVector())
	{
		...
		Matrix_elements.emplace_back(Triplet (cell_it.getId(), cell_it.getId(), porosity * cell_it.getVolume()));
	}
	...
	for (auto facet_it : this->M_mesh.getFractureFacetsIdsVector())
	{
		...
		Matrix_elements.emplace_back(Triplet (facet_it.getSeparated()[0], facet_it.getSeparated()[0], -porosity * volume / 2));
		...
		Matrix_elements.emplace_back(Triplet (facet_it.getSeparated()[1], facet_it.getSeparated()[1], -porosity * volume / 2));
	}
