...
int main(int argc, char * argv[])
{
    ...
    else if(data.getMeshType() == Data::MeshFormatType::Medit)
        importer = new ImporterMedit(data.getMeshDir() + data.getMeshFile(), mesh, propMap);
    else if(data.getMeshType() == Data::MeshFormatType::TetGen)
        importer = new ImporterTetGen(data.getMeshDir() + data.getMeshFile(), mesh, propMap);
    else if(data.getMeshType() == Data::MeshFormatType::OpenFOAM)
        importer = new ImporterOpenFOAM(data.getMeshDir() + data.getMeshFile(), mesh, propMap);
    std::cout << " done." << std::endl;
    ...
    if(data.getMeshType() == Data::MeshFormatType::Medit ||
       data.getMeshType() == Data::MeshFormatType::TetGen ||
       data.getMeshType() == Data::MeshFormatType::OpenFOAM)
    {
