...
namespace Geometry
{
  ...
  public:
    ...
    inline std::vector<Segment> getSx (gmm::size_type i) const
    {
      return (i == 1) ? M_S1x : M_S2x;
    }

    inline std::vector<Segment> getSy (gmm::size_type i) const
    {
      return (i == 1) ? M_S1y : M_S2y;
    }

    inline std::vector<Segment> getSz (gmm::size_type i) const
    {
      return (i == 1) ? M_S1z : M_S2z;
    }

    inline std::vector<gmm::size_type> M_ii() const
    {
      return M_ipos;
    }

    inline std::vector<gmm::size_type> M_jj() const
    {
      return M_jpos;
    }

    inline std::vector<gmm::size_type> M_kk() const
    {
      return M_kpos;
    }
    ...
    inline Real perm() const
    {
      return M_perm;
    }

    inline Real compr() const
    {
      return M_compr;
    }

    inline Real aperture() const
    {
      return M_aperture;
    }
    ...
    inline void setIsInt (gmm::size_type i)
    {
      M_isintby.push_back (i);
    }

    inline void clearIsInt()
    {
      M_isintby.resize (0);
    }
    ...
    inline std::vector<gmm::size_type> getIsInt() const
    {
      return M_isintby;
    }
    ...
    inline void setMetric (bool isMetric)
    {
      M_isMetric = isMetric;
      CDARCY = (M_isMetric == true) ? 0.008527 : 0.001127;
    }

    inline std::vector<IntFrac> inter()
    {
      return M_inter;
    }
    ...
    void setInterTransm (gmm::size_type quale, gmm::size_type dove, Real quanto)
    {
      M_inter[quale].M_Tf1f2[dove] = quanto;
    }
