...
//--------------segmenti di intersezione........................................
				...
				jt!=(*it).second.end(); ++jt)		
	{
		...
		{
			 puntiX1.push_back(jt->second); isXreal1.push_back(true); 
		}
...
std::cout << punti.size()<<std::endl;
	...
	Point3D CProp::setCG(Hull & calimero, gmm::size_type npunti_faglia){
		Point3D CG(0,0,0);
		Real Area(0);
		for (gmm::size_type i=0; i<calimero.getNtetra();++i){
			std::vector<gmm::size_type> punti_tetra(calimero.getPointsSimplex(i));
...
Point3D medio;
   ...
   if (cella.intersectTheFace(uno, ff,   medio)) {puntiareaNew.push_back(medio);}
   if (cella.intersectTheFace(due, ff,   medio)) {puntiareaNew.push_back(medio);}
   if (cella.intersectTheFace(tre, ff,   medio)) {puntiareaNew.push_back(medio);}
   if (cella.intersectTheFace(quattro, ff,   medio)) {puntiareaNew.push_back(medio);}
}
