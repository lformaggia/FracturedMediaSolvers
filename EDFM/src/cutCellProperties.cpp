...
//------------------costruttore 
	...
	CProp::CProp(const Intersect::GridIntersections & storage, CPgrid * gridpointer, Fracture * faultpointer){M_iteratorcellsbegin=storage.begin();
...
counter+=1;
		...
		{	M_Ne=M_Ne+1;
			...
			this->getCellPoints(it, punti1, 1, uv, Aa);	
			...
			this->getCellPoints(it, punti2, -1, uv, Aa);	
...
//--------------prendere per ogni cella i punti sopra o sotto la faglia-------------------
	...
	Point3D CProp::setCG(std::vector<Point3D> & punti){
		Point3D CG(0,0,0);
...
std::vector<Point3D> CProp::addPoints4area(std::vector<Point3D> & puntiarea, std::vector<bool> puntiIsReal, Intersect::GridIntersections_Const_Iterator_Type & it){
...
for (gmm::size_type i=0; i<npunti;++i){
	for (gmm::size_type j=i+1; j<npunti;++j){
		if (puntiIsReal[i]!=puntiIsReal[j] && !(puntiarea[i]==puntiarea[j]) ){
			...
			if (linea.intersectTheSegment(uno,medio)) {puntiareaNew.push_back(medio);}	
			if (linea.intersectTheSegment(due,medio)) {puntiareaNew.push_back(medio);}	
			if (linea.intersectTheSegment(tre,medio)) {puntiareaNew.push_back(medio);}
if (linea.intersectTheSegment(quattro,medio)) {puntiareaNew.push_back(medio);}
		}
	}
}
...
Point3D medio;
   ...
   if (cella.intersectTheFace(uno, ff,   medio)) {puntiareaNew.push_back(medio);}// std::cout << "1**"<<ff<<"  "<<medio<< std::endl;}
   if (cella.intersectTheFace(due, ff,   medio)) {puntiareaNew.push_back(medio);}// std::cout << "2**"<<ff<<"  "<<medio<< std::endl;}
   if (cella.intersectTheFace(tre, ff,   medio)) {puntiareaNew.push_back(medio);}// std::cout << "3**"<<ff<<"  "<<medio<< std::endl;}
   if (cella.intersectTheFace(quattro, ff,   medio)) {puntiareaNew.push_back(medio);}// std::cout << "4**"<<ff<<"  "<<medio<< std::endl;}
}
...
/*std::cout <<"--------"<<std::endl;
std::cout <<puntiareaNew.size()<<std::endl;
for (gmm::size_type i=0; i<puntiareaNew.size();++i){
	std::cout << puntiareaNew[i]<<std::endl;
}*/
